...
class CollaborationServer {
  ...
  private async handleJoinRoom(socket: any, data: { contractId: string; user: CollaborationUser }) {
    ...
    console.log(`User ${user.name} joined room ${contractId}`);
  }

  private async handleLeaveRoom(socket: any, data: { contractId: string; userId: string }) {
    ...
    if (room && room.users.has(userId)) {
      ...
      if (room.users.size === 0) {
        ...
        console.log(`Room ${contractId} cleaned up (no users)`);
      }

      console.log(`User ${user.name} left room ${contractId}`);
    }
  }

  private async handleDocumentChange(socket: any, data: DocumentChange) {
    ...
    if (room) {
      ...
      try {
        ...
        console.error('Failed to store document change:', error);
      }
      ...
      console.log(`Document change in room ${contractId}: ${data.changeType} by ${data.userName}`);
    }
  }

  private handleCursorMove(socket: any, data: { contractId: string; user: CollaborationUser; cursor: any }) {
  ...
  private handleTyping(socket: any, data: { contractId: string; userId: string; isTyping: boolean }) {
  ...
  private async handleAddComment(socket: any, data: { contractId: string; comment: any }) {
    ...
    console.log(`Comment added in room ${contractId}: ${comment.content}`);
  }
  ...
  private async createCollaborationNotification(contractId: string, user: CollaborationUser, action: 'joined' | 'left') {
    try {
      ...
      console.error('Failed to create collaboration notification:', error);
    }
  }
  ...
  private async createDocumentChangeNotification(contractId: string, change: DocumentChange) {
    try {
      ...
      console.error('Failed to create document change notification:', error);
    }
  }
