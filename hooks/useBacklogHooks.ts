...
export const useBacklogSummary = () =>
  useMutation({
    mutationFn: async (business: string) => {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL || "http://10.116.2.72:8091/api/backlog-summary", {
        method: "POST",
...
export const useBacklogCustomers = () =>
  useMutation({
    mutationFn: async (business: string) => {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL || "http://10.116.2.72:8091/api/customer-backlogs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ business }),
      });
  ...
  useMutation({
    mutationFn: async (business: string) => {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL || "http://10.116.2.72:8091/api/per-customer-backlogs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ business }),
      });
