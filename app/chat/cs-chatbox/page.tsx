...
export default function ToolSelectionPage() {
  const [selectedTool, setSelectedTool] = useState<string | null>('cs-agent');
  const [messages, setMessages] = useState<Message[]>([]);
  const [inputValue, setInputValue] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [isDarkMode, setIsDarkMode] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const webhookUrl = 'http://10.116.2.72:5678/webhook/cs-agent-prompt';
  ...
  const handleToolClick = (toolId: string) => {
    if (toolId === 'cs-agent') {
      setSelectedTool('cs-agent');
    } else {
      // For other tools, open in new tab
